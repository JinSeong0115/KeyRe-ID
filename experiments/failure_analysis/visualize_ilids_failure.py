"""iLIDS-VID Failure Case Visualization"""
...
def main():
    ...
    # Load model
    _, _, num_classes, cam_num, _, q_set, g_set = heatmap_dataloader(
        'iLIDSVID', './data', split_id=args.split_id)
    ...
    try:
        ...
        q_paths = [query_data[i][0] for i in range(len(query_data))]
        g_paths = [gallery_data[i][0] for i in range(len(gallery_data))]
    ...
    for i, (q_idx, q_pid, first_rank, top_gallery, top_correct) in enumerate(failures[:n_fail]):
        ...
        for j, g_idx in enumerate(top_gallery[:args.top_k]):
            ...
            try:
                g_img = load_and_resize(g_frame)
                axes[row][j+1].imshow(g_img)
            except:
    ...
    plt.tight_layout()
    save_path = os.path.join(args.output_dir, 'ilids_failure_analysis.png')
    plt.savefig(save_path, dpi=200, bbox_inches='tight')
    plt.close()
    print(f'Saved: {save_path}')

    # Save text summary
    with open(os.path.join(args.output_dir, 'ilids_failure_summary.txt'), 'w') as f:
        f.write(f'Rank-1: {cmc[0]:.4f}, Rank-5: {cmc[4]:.4f}, mAP: {mAP:.4f}\n')
        f.write(f'Total queries: {len(q_pids)}\n')
        f.write(f'Failures (rank-1 wrong): {len(failures)}\n\n')
        for i, (q_idx, q_pid, first_rank, _, _) in enumerate(failures[:10]):
            f.write(f'Failure #{i+1}: query_idx={q_idx}, true_pid={q_pid}, first_correct_rank={first_rank}\n')
    print('Summary saved.')
