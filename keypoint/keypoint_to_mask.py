...
def run_heatmap_generation(args):
    ...
    phases = [phase for phase in candidate_phases if os.path.isdir(os.path.join(image_root, phase))]
    if not phases:
        raise RuntimeError(f"No supported dataset split directories found under {image_root}")
    
    kp2mask = KeypointsToMasks(g_scale=11, vis_thresh=0.1, vis_continous=False)
    processed = 0
    missing_keypoint_dirs = 0
    ...
    for phase in phases:
        ...
        for root, _, files in os.walk(phase_img_dir):
            rel_dir = os.path.relpath(root, phase_img_dir)
            if rel_dir == ".":
                continue

            kp_dir = os.path.join(phase_kp_dir, rel_dir)
            if not os.path.exists(kp_dir):
                missing_keypoint_dirs += 1
                continue

            heatmap_dir = os.path.join(phase_heatmap_dir, rel_dir)
            ...
            for img_file in sorted(files):
                ...
                npy_save_path = os.path.join(heatmap_dir, f"{stem}.npy")
                if args.skip_existing and os.path.exists(npy_save_path):
                    processed += 1
                    continue
                np.save(npy_save_path, heatmap)
                processed += 1
                if args.log_every and processed % args.log_every == 0:
                    print(f"Processed {processed} heatmaps. Latest: {npy_save_path}")

    if missing_keypoint_dirs:
        print(f"Skipped {missing_keypoint_dirs} image directories without keypoints.")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_images', type=int, default=None, help='Optional limit for smoke tests')
    parser.add_argument('--skip_existing', action='store_true', help='Skip heatmaps that already exist')
    parser.add_argument('--log_every', type=int, default=100, help='Progress logging interval')
