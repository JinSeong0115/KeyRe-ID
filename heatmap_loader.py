...
__factory = {
    'MARS':MARS,
    'iLIDSVID':iLIDSVID,
    'DukeMTMC-VideoReID': DukeMTMCVideoReID,
    'DukeMTMCVideoReID': DukeMTMCVideoReID,
    'DukeVideoReID': DukeMTMCVideoReID,
    'Duke': DukeMTMCVideoReID
}
...
def _resolve_dataset_path(dataset_name, dataset_root):
    direct_path = os.path.abspath(os.path.expanduser(dataset_root))
    nested_path = os.path.join(direct_path, dataset_name)
...
def _resolve_heatmap_root(dataset_path, split_names):
    candidates = []
    for heatmap_dirname in ("heatmap", "heatmaps"):
        for split_name in split_names:
            candidates.append(os.path.join(dataset_path, heatmap_dirname, split_name))
...
def _heatmap_path_from_image(img_path, heatmap_root):
    stem = os.path.splitext(os.path.basename(img_path))[0]
    heatmap_name = f"{stem}.npy"
    split_name = os.path.basename(os.path.normpath(heatmap_root))
    path_parts = os.path.normpath(img_path).split(os.sep)

    if split_name in path_parts:
        split_idx = len(path_parts) - 1 - path_parts[::-1].index(split_name)
        rel_parts = path_parts[split_idx + 1:-1] + [heatmap_name]
        candidate = os.path.join(heatmap_root, *rel_parts)
        if os.path.exists(candidate):
            return candidate

    parent_name = os.path.basename(os.path.dirname(img_path))
    candidate = os.path.join(heatmap_root, parent_name, heatmap_name)
    if os.path.exists(candidate):
        return candidate

    grandparent_name = os.path.basename(os.path.dirname(os.path.dirname(img_path)))
    return os.path.join(heatmap_root, grandparent_name, parent_name, heatmap_name)

# heatmap_dataloader function
def heatmap_dataloader(Dataset_name, dataset_root):
    dataset_path = _resolve_dataset_path(Dataset_name, dataset_root)
    heatmap_train_root = _resolve_heatmap_root(dataset_path, ["bbox_train", "train", "bounding_box_train"])
    heatmap_query_root = _resolve_heatmap_root(dataset_path, ["bbox_test", "bbox_query", "query", "bounding_box_query"])
    heatmap_gallery_root = _resolve_heatmap_root(dataset_path, ["bbox_test", "bbox_gallery", "gallery", "bounding_box_test"])
    ...
    q_val_set = Heatmap_Dataset(
        ...
        heatmap_transform=CustomHeatmapTransform([256, 128]),
        heatmap_root=heatmap_query_root
    )
    g_val_set = Heatmap_Dataset(
        ...
        transform=val_transforms,
        heatmap_transform=CustomHeatmapTransform([256, 128]),
        heatmap_root=heatmap_gallery_root
    )
    ...
    def __getitem__(self, index):
        ...
        for i in range(clips):
            ...
            for frame_path in img_paths[start_idx:end_idx]:
                heatmap_file = _heatmap_path_from_image(frame_path, self.heatmap_root)
...
class Heatmap_Dataset_inderase(VideoDataset_inderase):
    ...
    def load_heatmap(self, img_path):
        if img_path not in self.heatmap_cache:
            heatmap_file = _heatmap_path_from_image(img_path, self.heatmap_root)
